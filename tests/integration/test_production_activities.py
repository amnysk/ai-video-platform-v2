...
@pytest.fixture
def acts(factory, store) -> ProductionActivities:
    return ProductionActivities(session_factory=factory, store=store, bucket=BUCKET)
...
async def test_admit_from_storyboard_ready_writes_token(acts, factory) -> None:
    episode_id = await _episode_at(factory, TO_STORYBOARD_READY)
...
async def test_record_failure_leaves_interrupted_jobs_resumable(acts, factory) -> None:
    episode_id = await _episode_at(factory, TO_STORYBOARD_READY)
    await _admit(acts, episode_id)
    async with factory() as session:
        jobs = JobRepository(session)
        running = await jobs.create(
            episode_id=episode_id, type=JobType.PRODUCE_SCENE_IMAGE, scene_id="sb1"
        )
        await jobs.start(running.id)
        queued = await jobs.create(
            episode_id=episode_id, type=JobType.PRODUCE_SCENE_VIDEO, scene_id="sb1"
        )
        await session.commit()

    await acts.record_failure(_failure(episode_id, FailureClass.NEEDS_INPUT))

    by_id = {j.id: j for j in await _jobs(factory, episode_id)}
    assert by_id[running.id].status is JobStatus.RETRYABLE_FAILED
    assert by_id[queued.id].status is JobStatus.RETRYABLE_FAILED
    async with factory() as session:  # 次の実行で start できる
        await JobRepository(session).start(running.id)
        await session.commit()
