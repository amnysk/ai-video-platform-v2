...
| `in_progress` | `uploaded` | 完成動画を private 投稿し受領 Artifact を保存した（`UPLOAD_SUCCEEDED`、ADR-0020） | upload workflow |
...
- `render_ready` → 次工程の開始（Phase 6 Upload の POST、ADR-0020。再描画の POST も出口、ADR-0019）
