...
## 追補3: 保証は制作工程へ（2026-09-20、ADR-0027）

上の予算（台本・storyboard の検査）は**推定**で、保証ではない。溢れを描画（有料の制作の後）まで
持ち越さないため、音声の合成後に実尺を区間と突き合わせ、上限つきの話速調整で合わせる検査を
制作工程（音声 Activity・マニフェスト組み立て）に置いた。本 ADR の予算検査は変えない（上流での
再生成という第一の手段のまま）。詳細は [ADR-0027](./0027-voice-fit-before-render.md)。
