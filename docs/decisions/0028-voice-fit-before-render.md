...
2. **音声 Activity が合成後に判定する**（`workers/production_voice/activities.py`、判定は純粋関数
   `domain/production/voice_fit.py`）。実尺 <= 区間ならそのまま。超えたら**実測を積んで**話速を上げ、合成し直す。
   尺は話速に反比例しない（追補: 実データ）ので、線形の外挿を信用しない:
   - 1 回目（実測 1 点）は線形の見積もり（区間の 97% に収まる量、`VOICE_FIT_HEADROOM_PERMILLE = 30`）。
     **見積もりが上限を超えても諦めず、上限で実測する**
   - 2 回目以降（実測 2 点以上）は直近 2 点に `尺 = 固定 + 可変 / 倍率` を当てはめて次の話速を決める。
     2 点が右下がりでない（ゆらぎ・縮まない生成器）、目標が固定部以下なら上限で確かめる。前より最低 20‰ は上げる
   - **最後に許された再合成は、途中の見積もりに関わらず必ず上限**（`MAX_VOICE_SPEEDUP_PERMILLE = 1250`、1.25 倍）
   - **合成は最大 4 回**（等速 1 + 再合成 `VOICE_FIT_MAX_RESYNTHESES = 3`）。`voice_concurrency` 1 で直列なので
     壁時計の上限でもある。収まった最初の実測を採るので、試した中で収まる最も遅い話速になる
   - **失敗（`VoiceExceedsSceneSpanError`）は「上限の話速で実測してなお区間を超えた」ときだけ**。
     外挿では諦めない。メッセージに全実測（話速‰ = 尺 ms）を入れる
   - 速さを変える生成器を持たない場合は、超過を即同じエラー（再合成しない）
...
- **線形の見積もりを 2 回繰り返す（初版）**: 9/19 s2 の実データで、上限 1250‰ なら収まる（8,975 ms）のに
  9,067 ms で失敗した（追補）。失敗の根拠が外挿になってしまう。実測を積み、最後は上限で確かめる方式に改めた
...
## 追補（2026-09-20）: 尺は話速に線形でない（実データ）

### Context

deployed Piper で 9/19 Episode のシーンを合成した実測（smoke）:

- 尺の約 2 割は話速で縮まない固定の間。同じ入力・同じ話速でも合成ごとに ±4% ゆらぐ
- s2（区間 9,000 ms、等速 10,658 ms）: 1177‰ → 9,601 ms、**1250‰（上限）→ 8,975 ms で収まる**。
  初版の「線形の見積もりで最大 2 回再合成」は 9,067 ms で終わり、上限なら収まるのに
  `VoiceExceedsSceneSpanError` にした

### Decision

上の Decision 2 のとおり、実測を積む探索にした（決定論的な純粋関数 `next_speed_permille`、入力は
`VoiceTake(話速, 尺)` の列）。要点: 1 点は線形・2 点以上は `固定 + 可変/倍率` の当てはめ・
最後の再合成は必ず上限・失敗は上限での実測が超えたときだけ・合成は最大 4 回。上限 1.25 倍・
`needs_input` の分類・尺の定数を持たない点は変えない。

### Consequences

- 良い: 失敗が「上限でも収まらない」という実測の事実になり、上限で収まる入力を取りこぼさない
- 悪い: 最悪 4 回合成する（初版は最大 3 回）。Piper の 1 回は数秒なので、5 シーン直列でも最悪 1〜2 分程度の増加
- 悪い: ゆらぎ ±4% があるので、上限での実測が区間ぎりぎり（±4% 以内）の入力は、同じ入力でも
  再実行で成否が変わりうる（ADR-0012 の非決定的な生成器として受け入れる）。上限を測って超えたときは
  `needs_input`（人間が台本を直すか、再実行）
- 負債: 当てはめは直近 2 点のみ（固定 + 可変/倍率）。3 点目以降の実測は使わない
