#!/usr/bin/env bash
# Production Image Worker を **ホストプロセス**として起動する（ADR-0017 Phase 4A）。
#
# 有料 provider（fal）を呼ぶ。FAL_KEY は .env かシェル環境に置く（コミットしない）。
#
#   docker compose --profile core up -d --wait        # 基盤
#   ./scripts/run-production-image-worker.sh          # 別ターミナルで worker
set -euo pipefail
...
echo "production image worker starting (host process)"
echo "  temporal  : $TEMPORAL_ADDRESS"
echo "  database  : ${DATABASE_URL%%://*}://<redacted>"
echo "  minio     : $MINIO_ENDPOINT"
echo "  work root : $AI_VIDEO_WORK_ROOT"
echo "  fal key   : <set>"
exec "$VENV/bin/python" -m workers.production_image.run_worker
